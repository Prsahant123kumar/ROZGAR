...
const EditWorkerProfile = ({ selectedMenu, editOpen, setEditOpen }) => {
  ...
  const { loading, EditWorkerProfile } = useMenuStore();
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      await EditWorkerProfile(selectedMenu._id, formData);
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <DialogContent>
        ...
        <form onSubmit={submitHandler} className="space-y-4">
          ...
          <div>
            ...
            />
            {error.image && <span className="text-xs font-medium text-red-600">{error.image}</span>}
          </div>
